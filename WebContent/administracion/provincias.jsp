...
if (request.getAttribute("rs")!=null){
	...
	provincias = (Provincia[])request.getAttribute("rs");
...
if(provincias != null){
	...
	out.print("		<table class=\"styled-table\">\n"
		...
		indice = 0;
	while(indice < provincias.length)
	{
		...
		if(provincias[indice] != null){
			...
			out.println("<td>"+provincias[indice].getId_provincia()+"</td>");
			out.println("<td>"+provincias[indice].getNombre()+"</td>");

			out.println("<td>");
			out.println("<form method='post' action='/cinexin/administracion/eliminarprovincia'>");
			out.println("<input type='submit' value='Borrar'/>");
			out.println("<input type='hidden' value='" + provincias[indice].getId_provincia() + "' name='id_provincia' />");
			out.println("</form></td>");
			
			//Para modificar el montadito llamamos a otro jsp pasandole el id de montadito
			out.println("</td>");
			out.println("<td>");
			out.println("<form method='get' action='/cinexin/administracion/modificarprovincia'>");
			out.println("<input type='submit' value='Modificar'/>");
			out.println("<input type='hidden' value='" + provincias[indice].getId_provincia()+ "' name='id_provincia' />");
			out.println("<input type='hidden' value='" + provincias[indice].getNombre() + "' name='nombre' />");
	...
	out.print("<h2>No hay provincias para mostrar</h2>");
	out.print("<a href='/cinexin/administracion/provincias'>Prueba de nuevo pulsando aquí.</a><br/>");
}
...
if(pagina > 1){
	out.print("<a href='/cinexin/administracion/provincias?page="+(pagina - 1) +"'> &#60;</a>");
}
...
out.println("<a href='/cinexin/administracion/provincias?page="+(pagina + 1) +"'> &#62;</a> ");
}
