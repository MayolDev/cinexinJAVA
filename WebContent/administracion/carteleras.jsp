...
if (request.getAttribute("rs")!=null){
	...
	carteleras = (Cartelera[])request.getAttribute("rs");
...
if(carteleras != null){
	...
	out.print("		<table class=\"styled-table\">\n"
		...
		indice = 0;
	while(indice < carteleras.length)
	{
		
		if(carteleras[indice] != null){
			
			out.println("<tr>");
			//Mostramos en cada celda td los datos correspondientes a su columna
			out.println("<td>"+carteleras[indice].getId()+"</td>");
			out.println("<td>"+carteleras[indice].getNombre()+"</td>");


			out.println("<td>");
			out.println("<form method='post' action='/cinexin/administracion/eliminarcartelera'>");
			out.println("<input type='submit' value='Borrar'/>");
			out.println("<input type='hidden' value='" + carteleras[indice].getId() + "' name='id' />");
			out.println("</form></td>");
			
			//Para modificar el montadito llamamos a otro jsp pasandole el id de montadito
			out.println("</td>");
			out.println("<td>");
			out.println("<form method='get' action='/cinexin/administracion/modificarcartelera'>");
			out.println("<input type='submit' value='Modificar'/>");
			out.println("<input type='hidden' value='" + carteleras[indice].getId() + "' name='id' />");
			out.println("<input type='hidden' value='" + carteleras[indice].getNombre()+ "' name='nombre' />");
	...
	out.print("<a href='/cinexin/administracion/carteleras'>Prueba de nuevo pulsando aquí.</a><br/>");
}
...
if(pagina > 1){
	out.print("<a href='/cinexin/administracion/carteleras?page="+(pagina - 1) +"'> &#60;</a>");
}
...
out.println("<a href='/cinexin/administracion/carteleras?page="+(pagina + 1) +"'> &#62;</a> ");
}
