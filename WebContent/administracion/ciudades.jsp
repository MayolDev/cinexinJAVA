...
if (request.getAttribute("rs")!=null){
	...
	ciudades = (Ciudad[])request.getAttribute("rs");
...
if(ciudades != null){
	...
	while(indice < ciudades.length)
	{
		...
		if(ciudades[indice] != null){
			
			out.println("<tr>");
			//Mostramos en cada celda td los datos correspondientes a su columna
			out.println("<td>"+ciudades[indice].getCod_postal()+"</td>");
			out.println("<td>"+ciudades[indice].getId_provincia()+"</td>");
			out.println("<td>"+ciudades[indice].getNombre()+"</td>");


			out.println("<td>");
			out.println("<form method='post' action='/cinexin/administracion/eliminarciudad'>");
			out.println("<input type='submit' value='Borrar'/>");
			out.println("<input type='hidden' value='" + ciudades[indice].getCod_postal() + "' name='cod_postal' />");
			out.println("</form></td>");
			
			//Para modificar el montadito llamamos a otro jsp pasandole el id de montadito
			out.println("</td>");
			out.println("<td>");
			out.println("<form method='get' action='/cinexin/administracion/modificarciudad'>");
			out.println("<input type='submit' value='Modificar'/>");
			out.println("<input type='hidden' value='" + ciudades[indice].getCod_postal() + "' name='cod_postal' />");
			out.println("<input type='hidden' value='" + ciudades[indice].getId_provincia()+ "' name='id_provincia' />");
			out.println("<input type='hidden' value='" + ciudades[indice].getNombre() + "' name='nombre' />");
	...
	out.print("<h2>No hay ciudades para mostrar</h2>");
	out.print("<a href='/cinexin/administracion/ciudades'>Prueba de nuevo pulsando aquí.</a><br/>");
}
...
if(pagina > 1){
	out.print("<a href='/cinexin/administracion/ciudades?page="+(pagina - 1) +"'> &#60;</a>");
}
...
out.println("<a href='/cinexin/administracion/ciudades?page="+(pagina + 1) +"'> &#62;</a> ");
}
