...
if(request.getParameter("cod_postal") == null){
	...
	response.sendRedirect("/administracion/ciudades");
}

String cod_postal, nombre;
int id_provincia;

cod_postal = (String)request.getAttribute("cod_postal");
...
<input type='hidden' value="<%=cod_postal %>" name='cod_postal' />
<label>Introduce nombre</label>
<input name="nombre" type="text" value="<%=nombre %>"/>
<label>Introduce id_provincia</label>
<input name="id_provincia" type="number" value="<%=id_provincia %>"/>
